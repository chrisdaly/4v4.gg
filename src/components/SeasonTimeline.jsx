    ...
    <div>
      ...
      <div style={{ position: "relative", height: 20, marginBottom: 6 }}>
        {yearLabels.map(({ label, date }) => {
          ...
          return (
            <div key={label} style={{ position: "absolute", left: `${pct}%`, transform: "translateX(-50%)", fontFamily: "var(--font-mono)", fontSize: 12, color: chartColors.label, whiteSpace: "nowrap" }}>
              {label}
            </div>
          );
        })}
      ...
      <div style={{ position: "relative", height: 80 }}>
        {/* Baseline */}
        <div style={{ position: "absolute", top: "50%", left: 0, right: 0, height: 1, background: chartColors.grid, transform: "translateY(-50%)" }} />
        ...
        {usedSeasons.map(s => {
          ...
          if (pct === null) return null;
          return (
            <div key={s} style={{ position: "absolute", left: `${pct}%`, top: "10%", height: "80%", width: 1, background: chartColors.grid }} />
          );
        })}
        ...
        {seasonActivity.map(({ season }) => {
          ...
          if (pct === null) return null;
          return (
            <div key={season} style={{ position: "absolute", left: `${pct + 0.3}%`, bottom: 6, fontFamily: "var(--font-mono)", fontSize: 11, color: colors.gold.value, opacity: 0.75, whiteSpace: "nowrap" }}>
              S{season}
            </div>
          );
        })}
